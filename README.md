...
### 1. CSP(Constraint Satisfaction Problem) 알고리즘
단순 Brute-Force 방식이 아닌 **백트래킹(Backtracking)과 AC-3 알고리즘, MRV 휴리스틱**을 적용하여 탐색 공간을 수십만 배 이상 축소했습니다. 이를 통해 수억 가지 이상의 경우의 수도 수 초 내에 최적해로 수렴하여 기존 서비스의 탐색 한계를 극복했습니다.

### 2. 효율적인 캐싱 메커니즘 (Gemini API 최적화)
사용자의 비규격 자연어 입력을 매번 Gemini API로 파싱할 경우 발생하는 API 호출 제한(Rate Limit) 문제와 리소스 낭비를 방지하기 위해 **유사도 기반 캐싱 시스템**을 도입했습니다.

* **동작 원리**: 
    1. 사용자의 입력이 들어오면 기존에 저장된 문장 데이터셋과 **코사인 유사도(Cosine Similarity) 또는 레벤슈타인 거리(Levenshtein Distance)**를 계산합니다.
    2. 유사도 기준값(Threshold) 이상인 문장이 존재할 경우, AI 호출 없이 **즉시 캐시된 결과를 반환**합니다.
    3. 새로운 형태의 입력일 경우에만 Gemini API를 호출하여 결과를 생성하고, 이후 재사용을 위해 캐시에 저장합니다.
* **기대 효과**: API 호출 횟수를 획기적으로 절감하여 서비스의 비용 효율성을 높이고, 반복적인 조건 입력에 대해 0.1초 내외의 초고속 응답을 제공합니다.
